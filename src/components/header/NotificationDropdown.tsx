...
export default function NotificationDropdown({
  ...
  return (
    <div className="relative">
      ...
      >
        {hasUnread && (
          <span className="absolute -top-1 -right-1 z-10 inline-flex min-w-5 h-5 items-center justify-center rounded-full bg-orange-500 px-1 text-[10px] font-semibold text-white">
            {unreadLabel}
          </span>
        )}
        <svg
        ...
        {notifications.length === 0 ? (
          ...
          <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
            {notifications.map((notification) => (
              <li key={`${notification.id}-${notification.createdAt}`}>
                <DropdownItem
                  tag="a"
                  to="/orders"
                ...
                >
                  <span className="block">
                    <span className="mb-1 block text-theme-sm text-gray-800 dark:text-white/90">
                      Order <span className="font-semibold">#{notification.orderCode}</span> by{" "}
                      <span className="font-medium">{notification.customerLabel}</span>
                    </span>
